...
(0, node_test_1.describe)('PullDirect', async () => {
    ...
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.GithubRestSDK({
    ...
    (0, node_test_1.test)('direct-load-pull', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-pull', setup.live))
            return;
    ...
    (0, node_test_1.test)('direct-list-pull', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-pull', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["owner01", "repo01"]))
            return;
        const { client, calls } = setup;
...
function liveScenariosActive() { return false && process.env.GITHUB_REST_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    const calls = [];
    const env = (0, utility_1.envOverride)({
        'GITHUB_REST_TEST_PULL_ENTID': {},
    ...
    if (live) {
        ...
        const client = new __1.GithubRestSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
            apikey: env.GITHUB_REST_APIKEY,
        }));
        let idmap = env['GITHUB_REST_TEST_PULL_ENTID'];
    ...
    const client = new __1.GithubRestSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=PullDirect.test.js.map