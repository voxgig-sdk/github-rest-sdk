...
//     (\`new GithubRestSDK(...)\` / \`GithubRestSDK.test(...)\` become
//     \`GithubRestSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"branch":{"test01":{"id":"test01"}},"commit":{"test01":{"id":"test01"}},"gist":{"test01":{"id":"test01"}},"issue":{"test01":{"id":"test01"}},"notification":{"test01":{"id":"test01"}},"org":{"test01":{"id":"test01"}},"pull":{"test01":{"id":"test01"}},"rate_limit":{"test01":{"id":"test01"}},"repo":{"test01":{"id":"test01"}},"search":{"test01":{"id":"test01"}},"user":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new GithubRestSDK(...)\` and any
// \`GithubRestSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+GithubRestSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/GithubRestSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
