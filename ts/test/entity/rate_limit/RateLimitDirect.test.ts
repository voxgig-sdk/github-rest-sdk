...
describe('RateLimitDirect', async () => {
  ...
  test('direct-load-rate_limit', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-rate_limit', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITHUB_REST_TEST_RATE_LIMIT_ENTID']
