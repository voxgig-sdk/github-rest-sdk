...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.GITHUBREST_TEST_LIVE ||
    'TRUE' === process.env.GITHUBREST_TEST_OVERRIDE
  ) {
  ...
  m.GITHUBREST_TEST_EXPLAIN = process.env.GITHUBREST_TEST_EXPLAIN || m.GITHUBREST_TEST_EXPLAIN
