...
The Lua SDK for the GithubRest API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Branch(nil):list(nil, nil)
...
local result, err = client:GithubRest(nil):load(
  { id = "test01" }, nil
)
