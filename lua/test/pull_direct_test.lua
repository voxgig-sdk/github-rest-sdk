...
describe("PullDirect", function()
  it("should direct-list-pull", function()
    local setup = pull_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-pull", setup.live and "live" or "unit")
  ...
  it("should direct-load-pull", function()
    local setup = pull_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-pull", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function pull_direct_setup(mockres)
