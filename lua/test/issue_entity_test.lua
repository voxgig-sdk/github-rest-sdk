...
describe("IssueEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Issue(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Issue(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set GITHUB_REST_TEST_ISSUE_ENTID JSON to run live")
      return
    end
    ...
    assert.is_nil(err)
    issue_ref01_data = helpers.to_map(type(issue_ref01_data_result) == 'table' and issue_ref01_data_result.data_get and issue_ref01_data_result:data_get() or issue_ref01_data_result)
    ...
    assert.is_nil(err)
    local issue_ref01_resdata_up0 = helpers.to_map(type(issue_ref01_resdata_up0_result) == 'table' and issue_ref01_resdata_up0_result.data_get and issue_ref01_resdata_up0_result:data_get() or issue_ref01_resdata_up0_result)
    ...
    assert.is_nil(err)
    local issue_ref01_data_dt0_load_result = helpers.to_map(type(issue_ref01_data_dt0_loaded) == 'table' and issue_ref01_data_dt0_loaded.data_get and issue_ref01_data_dt0_loaded:data_get() or issue_ref01_data_dt0_loaded)
...
function issue_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("GITHUB_REST_TEST_ISSUE_ENTID")
  ...
  local idmap_resolved = helpers.to_map(
    env["GITHUB_REST_TEST_ISSUE_ENTID"])
