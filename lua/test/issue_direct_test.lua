...
describe("IssueDirect", function()
  it("should direct-list-issue", function()
    local setup = issue_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-issue", setup.live and "live" or "unit")
  ...
  it("should direct-load-issue", function()
    local setup = issue_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-issue", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function issue_direct_setup(mockres)
