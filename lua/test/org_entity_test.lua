...
describe("OrgEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set GITHUBREST_TEST_ORG_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local org_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.org")))
    local org_ref01_data = nil
    if #org_ref01_data_raw > 0 then
      org_ref01_data = helpers.to_map(org_ref01_data_raw[1][2])
    end
    ...
    assert.is_nil(err)
    local org_ref01_data_dt0_load_result = helpers.to_map(org_ref01_data_dt0_loaded)
...
function org_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("GITHUBREST_TEST_ORG_ENTID")
