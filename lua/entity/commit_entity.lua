-- GithubRest SDK Commit entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local CommitEntity = {}
CommitEntity.__index = CommitEntity


function CommitEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CommitEntity)
  self._name = "commit"
  self._client = client
...
function CommitEntity:get_name()
  return self._name
end


function CommitEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CommitEntity.new(self._client, opts)
end
...
function CommitEntity:mark_deleted()
  self._deleted = true
end


function CommitEntity:deleted()
  return true == self._deleted
end


function CommitEntity:data_set(args)
  if args ~= nil then
...
function CommitEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CommitEntity:match_set(args)
  if args ~= nil then
...
function CommitEntity:match_get()
...
function CommitEntity:stream(action, args, callopts)
...
---@param reqmatch CommitListMatch
---@param ctrl? table
---@return Commit[]
---@return string? err
function CommitEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function CommitEntity:_run_op(ctx, post_done)
  local utility = self._utility
