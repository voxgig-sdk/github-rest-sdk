...
local GistEntity = {}
GistEntity.__index = GistEntity


function GistEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GistEntity)
  self._name = "gist"
  self._client = client
...
function GistEntity:get_name()
  return self._name
end


function GistEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GistEntity.new(self._client, opts)
end
...
function GistEntity:mark_deleted()
  self._deleted = true
end


function GistEntity:deleted()
  return true == self._deleted
end


function GistEntity:data_set(args)
  if args ~= nil then
...
function GistEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GistEntity:match_set(args)
  if args ~= nil then
...
function GistEntity:stream(action, args, callopts)
...
function GistEntity:_run_op(ctx, post_done)
  local utility = self._utility
