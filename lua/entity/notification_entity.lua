...
NotificationEntity.__index = NotificationEntity


function NotificationEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, NotificationEntity)
...
function NotificationEntity:get_name()
  return self._name
end


function NotificationEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return NotificationEntity.new(self._client, opts)
end
...
function NotificationEntity:mark_deleted()
  self._deleted = true
end


function NotificationEntity:deleted()
  return true == self._deleted
end


function NotificationEntity:data_set(args)
  if args ~= nil then
...
function NotificationEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function NotificationEntity:match_set(args)
  if args ~= nil then
...
function NotificationEntity:match_get()
...
function NotificationEntity:stream(action, args, callopts)
...
function NotificationEntity:_run_op(ctx, post_done)
  local utility = self._utility
