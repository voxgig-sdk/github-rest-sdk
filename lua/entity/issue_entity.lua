-- GithubRest SDK Issue entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local IssueEntity = {}
IssueEntity.__index = IssueEntity


function IssueEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, IssueEntity)
  self._name = "issue"
  self._client = client
...
function IssueEntity:get_name()
  return self._name
end


function IssueEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return IssueEntity.new(self._client, opts)
end
...
function IssueEntity:mark_deleted()
  self._deleted = true
end


function IssueEntity:deleted()
  return true == self._deleted
end


function IssueEntity:data_set(args)
  if args ~= nil then
...
function IssueEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function IssueEntity:match_set(args)
  if args ~= nil then
...
function IssueEntity:match_get()
...
function IssueEntity:stream(action, args, callopts)
...
function IssueEntity:_run_op(ctx, post_done)
  local utility = self._utility
