...
local PullEntity = {}
PullEntity.__index = PullEntity


function PullEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PullEntity)
  self._name = "pull"
  self._client = client
...
function PullEntity:get_name()
  return self._name
end


function PullEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PullEntity.new(self._client, opts)
end
...
function PullEntity:mark_deleted()
  self._deleted = true
end


function PullEntity:deleted()
  return true == self._deleted
end


function PullEntity:data_set(args)
  if args ~= nil then
...
function PullEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PullEntity:match_set(args)
  if args ~= nil then
...
function PullEntity:stream(action, args, callopts)
...
function PullEntity:_run_op(ctx, post_done)
  local utility = self._utility
