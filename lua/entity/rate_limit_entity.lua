...
local RateLimitEntity = {}
RateLimitEntity.__index = RateLimitEntity


function RateLimitEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RateLimitEntity)
  self._name = "rate_limit"
  self._client = client
...
function RateLimitEntity:get_name()
  return self._name
end


function RateLimitEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RateLimitEntity.new(self._client, opts)
end
...
function RateLimitEntity:mark_deleted()
  self._deleted = true
end


function RateLimitEntity:deleted()
  return true == self._deleted
end


function RateLimitEntity:data_set(args)
  if args ~= nil then
...
function RateLimitEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RateLimitEntity:match_set(args)
  if args ~= nil then
...
function RateLimitEntity:match_get()
...
function RateLimitEntity:stream(action, args, callopts)
...
function RateLimitEntity:_run_op(ctx, post_done)
  local utility = self._utility
