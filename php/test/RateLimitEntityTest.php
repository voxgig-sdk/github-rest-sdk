...
class RateLimitEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set GITHUB_REST_TEST_RATE_LIMIT_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $rate_limit_ref01_data_dt0_loaded = $rate_limit_ref01_ent->load($rate_limit_ref01_match_dt0, null);
...
function rate_limit_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("GITHUB_REST_TEST_RATE_LIMIT_ENTID");
