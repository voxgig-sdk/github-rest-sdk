...
class GistEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Gist(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Gist(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set GITHUB_REST_TEST_GIST_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $gist_ref01_data = Helpers::to_map(is_object($gist_ref01_data_result) && method_exists($gist_ref01_data_result, 'data_get') ? $gist_ref01_data_result->data_get() : $gist_ref01_data_result);
...
function gist_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("GITHUB_REST_TEST_GIST_ENTID");
