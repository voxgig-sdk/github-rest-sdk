...
class CommitEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = GithubRestSDK::test(null, null);
        $ent = $testsdk->Commit(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Commit(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Commit(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = commit_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "commit." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        $commit_ref01_data = null;
        if (count($commit_ref01_data_raw) > 0) {
            $commit_ref01_data = Helpers::to_map($commit_ref01_data_raw[0][1]);
        }

        // LIST
        $commit_ref01_ent = $client->Commit(null);
        $commit_ref01_match = [
...
function commit_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/commit/CommitTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["commit01", "commit02", "commit03", "repo01", "repo02", "repo03", "owner01"] as $k) {
