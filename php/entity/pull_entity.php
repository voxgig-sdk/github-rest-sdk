...
class PullEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new PullEntity($this->_client, $opts);
    }

    /**
     * @param Pull|array $args Pull data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = GithubRestHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Pull|array The current Pull data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Pull fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = GithubRestHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Pull fields).
     */
     ...
     * Load a single Pull.
     *
     * @param PullLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed PullLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Pull|array The loaded Pull as an assoc-array at the
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = GithubRestHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
    ...
    /**
     * List Pull items matching the given filter.
     *
     * @param PullListMatch|array|null $reqmatch Match filter (any subset
     *   of Pull fields) as an assoc-array; PullListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Pull[]|array A list of Pull items as assoc-arrays at
     ...
     * Create a new Pull.
     *
     * @param PullCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed PullCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Pull|array The created Pull as an assoc-array at the
     *   SDK boundary; throws GithubRestError on failure (item-5 convention).
     */
