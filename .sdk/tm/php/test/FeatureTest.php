...
class FtCtrl extends GithubRestControl
{
...
class FtClient
{
    ...
    private GithubRestUtility $utility;

    public function __construct(GithubRestUtility $utility, array $options)
    {
    ...
    public function get_utility(): GithubRestUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (GithubRestContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public GithubRestContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (GithubRestContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = GithubRestConfig::make_feature($name);
    ...
    private static function buildUrl(GithubRestSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof GithubRestError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new GithubRestResponse($fetched);
        }
    ...
    private function fail(GithubRestContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (GithubRestContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
