...
class NotificationEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return NotificationEntity(self._client, opts)
    ...
    def data_get(self) -> Notification:
    ...
    def match_get(self) -> Notification:
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Notification]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Notification().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
