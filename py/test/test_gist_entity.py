...
class TestGistEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Gist(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Gist(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _gist_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create", "list"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "gist." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set GITHUB_REST_TEST_GIST_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        gist_ref01_ent = client.Gist(None)
        gist_ref01_data = helpers.to_map(vs.getprop(
            vs.getpath(setup["data"], "new.gist"), "gist_ref01"))

        gist_ref01_data = helpers.to_map(runner.entity_data(gist_ref01_ent.create(gist_ref01_data, None)))
...
def _gist_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/gist/GistTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("GITHUB_REST_TEST_GIST_ENTID"))
