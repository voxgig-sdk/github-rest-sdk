...
class TestGistDirect:

    def test_should_direct_list_gist(self):
        setup = _gist_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-gist", "live" if setup["live"] else "unit")
        if _skip:
...
def _gist_direct_setup(mockres):
