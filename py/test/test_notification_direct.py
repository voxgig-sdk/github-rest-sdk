...
class TestNotificationDirect:
    ...
    def test_should_direct_list_notification(self):
        setup = _notification_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-notification", "live" if setup["live"] else "unit")
        if _skip:
...
def _notification_direct_setup(mockres):
