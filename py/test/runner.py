...
class GithubRestTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            GithubRestTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = GithubRestTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = GithubRestTestRunner.getenv("GITHUB_REST_TEST_LIVE")
        override = GithubRestTestRunner.getenv("GITHUB_REST_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = GithubRestTestRunner.getenv(key)
        ...
        explain = GithubRestTestRunner.getenv("GITHUB_REST_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if GithubRestTestRunner._test_control is not None:
            return GithubRestTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                GithubRestTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            GithubRestTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = GithubRestTestRunner.load_test_control()
...
def load_env_local():
    GithubRestTestRunner.load_env_local()


def env_override(m):
    return GithubRestTestRunner.env_override(m)


def entity_data(v):
    return GithubRestTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return GithubRestTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return GithubRestTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return GithubRestTestRunner.load_test_control()


def live_client_options():
    return GithubRestTestRunner.live_client_options()


def live_delay_ms():
    return GithubRestTestRunner.live_delay_ms()
