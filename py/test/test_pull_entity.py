...
class TestPullEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Pull(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Pull(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set GITHUB_REST_TEST_PULL_ENTID JSON to run live")
        client = setup["client"]
        ...
        pull_ref01_data = helpers.to_map(runner.entity_data(pull_ref01_ent.create(pull_ref01_data, None)))
        ...
        pull_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(pull_ref01_data_dt0_loaded))
...
def _pull_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("GITHUB_REST_TEST_PULL_ENTID"))
