...
class PullEntity
  ...
  def make
    opts = @_entopts.dup
    PullEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = GithubRestHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Pull, Hash] the current Pull data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = GithubRestHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Pull fields)
  def match_get
  ...
  # Load a single Pull.
  #
  # @param reqmatch [PullLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Pull.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Pull, Hash] the loaded Pull; raises GithubRestError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = GithubRestHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [PullListMatch, Hash, nil] match filter (any subset of
  #   Pull fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Pull>, Array] the matching Pull items; raises GithubRestError on failure
  ...
  # Create a new Pull.
  #
  # @param reqdata [PullCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [Pull, Hash] the created Pull; raises GithubRestError on failure
