...
class RateLimitEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = rate_limit_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "rate_limit." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set GITHUB_REST_TEST_RATE_LIMIT_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    rate_limit_ref01_data = nil
    if rate_limit_ref01_data_raw.length > 0
...
def rate_limit_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "rate_limit", "RateLimitTestData.json")
  ...
  entid_env_raw = ENV["GITHUB_REST_TEST_RATE_LIMIT_ENTID"]
