...
class BranchDirectTest < Minitest::Test
  def test_direct_list_branch
    setup = branch_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-branch", setup[:live] ? "live" : "unit")
    if _should_skip
...
def branch_direct_setup(mockres)
