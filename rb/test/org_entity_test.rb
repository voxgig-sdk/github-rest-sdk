...
class OrgEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set GITHUB_REST_TEST_ORG_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    org_ref01_data = nil
    if org_ref01_data_raw.length > 0
...
def org_basic_setup(extra)
  ...
  entid_env_raw = ENV["GITHUB_REST_TEST_ORG_ENTID"]
