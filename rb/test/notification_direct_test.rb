...
class NotificationDirectTest < Minitest::Test
  def test_direct_list_notification
    setup = notification_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-notification", setup[:live] ? "live" : "unit")
    if _should_skip
...
def notification_direct_setup(mockres)
