...
class RepoEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set GITHUB_REST_TEST_REPO_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    repo_ref01_data = nil
    if repo_ref01_data_raw.length > 0
...
def repo_basic_setup(extra)
  ...
  entid_env_raw = ENV["GITHUB_REST_TEST_REPO_ENTID"]
