...
class CommitDirectTest < Minitest::Test
  def test_direct_list_commit
    setup = commit_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-commit", setup[:live] ? "live" : "unit")
    if _should_skip
...
def commit_direct_setup(mockres)
