...
module GithubRestUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = GithubRestResponse.new({})
    ...
    if fetch_err
      ...
      response = GithubRestResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = GithubRestResponse.new(fetched)
    else
