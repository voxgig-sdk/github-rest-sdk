...
module GithubRestUtilities
  ...
  TransformRequest = ->(ctx) {
    ...
    transform = GithubRestHelpers.to_map(VoxgigStruct.getprop(point, "transform"))
    return GithubRestUtilities.strip_action(ctx.reqdata) unless transform
    reqform = VoxgigStruct.getprop(transform, "req")
    return GithubRestUtilities.strip_action(ctx.reqdata) unless reqform
    GithubRestUtilities.strip_action(VoxgigStruct.transform({ "reqdata" => ctx.reqdata }, reqform))
  }
end
