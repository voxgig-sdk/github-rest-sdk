...
class GithubRestSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = GithubRestHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = GithubRestHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, GithubRestFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    GithubRestUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    params = GithubRestHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = GithubRestHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = GithubRestHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = GithubRestError.new(
        "graphql_error", "GithubRestSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
