...
boru-driven command-line client **and** interactive REPL for the GithubRest
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/github-rest-cli)
make build
...
./github-rest-cli list commit

# 5. Override the API base URL for a single call
GITHUB_REST_BASE=https://api.example.com ./github-rest-cli list branch

# 6. No arguments -> interactive REPL
./github-rest-cli
github-rest> list branch
github-rest> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/github-rest-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/github-rest-cli list branch
   ```
...
Run with no arguments to open a REPL (prompt `github-rest>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `branch`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/github-rest-cli`. |
...
(`./github-rest-cli <expr>`) and interactively (the REPL), and expressions compose the
