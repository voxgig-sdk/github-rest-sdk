...
type Args struct {
	Entity string         `json:"entity" jsonschema:"branch | commit | gist | issue | notification | org | pull | rate_limit | repo | search | user"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.GithubRestSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "github-rest_list",
		Description: "List records from GithubRest. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "github-rest_load",
		Description: "Load a single record from GithubRest. " +
...
func runOp(client *sdk.GithubRestSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.GithubRestSDK, name string) (sdk.GithubRestEntity, error) {
