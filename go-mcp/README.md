...
[MCP](https://modelcontextprotocol.io) server exposing the GithubRest SDK as
two agent tools — `github-rest_list` and `github-rest_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/github-rest-mcp)
make build

# 2. Provide credentials via the environment
export GITHUB_REST_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user github-rest \
  -- /absolute/path/to/github-rest-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./github-rest-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// github-rest_list: first page of records
{ "entity": "branch" }
{ "entity": "branch", "query": { } }

// github-rest_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/github-rest-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user github-rest \
     -- "$PWD"/dist/*/github-rest-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `github-rest_list` and `github-rest_load` tools now appear
   in new sessions. Ask the agent to *"list branch using github-rest"*
   and it calls `github-rest_list` with `{"entity":"branch"}`.
...
```sh
export GITHUB_REST_APIKEY=sk_live_xxx            # API key
export GITHUB_REST_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `github-rest_list` tool
...
### Call the `github-rest_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `github-rest_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `github-rest_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `GITHUB_REST_APIKEY` | API key sent with every request. |
| `GITHUB_REST_BASE` | Optional override of the API base URL. |

### Entities

The 11 entities valid as the `entity` argument:

branch | commit | gist | issue | notification | org | pull | rate_limit | repo | search | user

### Smoke test via HTTP (raw JSON-RPC)

```sh
./github-rest-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"github-rest_load","arguments":{"entity":"issue","query":{"id":1}}}}'
```
