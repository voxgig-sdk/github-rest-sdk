...
func TestGistEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Gist(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Gist(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set GITHUB_REST_TEST_GIST_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		gistRef01Data = core.ToMapAny(entityData(gistRef01DataResult))
...
func gistBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("GITHUB_REST_TEST_GIST_ENTID")
