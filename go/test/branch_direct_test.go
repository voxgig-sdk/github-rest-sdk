...
func TestBranchDirect(t *testing.T) {
	t.Run("direct-list-branch", func(t *testing.T) {
		setup := branchDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-branch", _mode); _shouldSkip {
			if _reason == "" {
...
type branchDirectSetupResult struct {
...
func branchDirectSetup(mockres any) *branchDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"GITHUBREST_TEST_BRANCH_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["GITHUBREST_TEST_BRANCH_ENTID"]; ok {
		...
		return &branchDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &branchDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
