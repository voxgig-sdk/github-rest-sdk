...
func TestOrgEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set GITHUBREST_TEST_ORG_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		orgRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.org", setup.data)))
		var orgRef01Data map[string]any
		if len(orgRef01DataRaw) > 0 {
			orgRef01Data = core.ToMapAny(orgRef01DataRaw[0][1])
		}
		...
		orgRef01DataDt0LoadResult := core.ToMapAny(orgRef01DataDt0Loaded)
...
func orgBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("GITHUBREST_TEST_ORG_ENTID")
	...
	idmapResolved := core.ToMapAny(env["GITHUBREST_TEST_ORG_ENTID"])
