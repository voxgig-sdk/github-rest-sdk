...
func TestPullDirect(t *testing.T) {
	t.Run("direct-list-pull", func(t *testing.T) {
		setup := pullDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-pull", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-pull", func(t *testing.T) {
		setup := pullDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-pull", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type pullDirectSetupResult struct {
...
func pullDirectSetup(mockres any) *pullDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"GITHUB_REST_TEST_PULL_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["GITHUB_REST_TEST_PULL_ENTID"]; ok {
		...
		return &pullDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &pullDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
