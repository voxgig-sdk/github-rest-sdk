...
func TestCommitEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set GITHUB_REST_TEST_COMMIT_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		commitRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.commit", setup.data)))
		var commitRef01Data map[string]any
		if len(commitRef01DataRaw) > 0 {
			commitRef01Data = core.ToMapAny(commitRef01DataRaw[0][1])
		}
		...
		_, commitRef01ListOk := commitRef01ListResult.([]any)
...
func commitBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap := vs.Transform(
		[]any{"commit01", "commit02", "commit03", "repo01", "repo02", "repo03", "owner01"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("GITHUB_REST_TEST_COMMIT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["GITHUB_REST_TEST_COMMIT_ENTID"])
